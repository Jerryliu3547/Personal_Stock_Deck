...
stock_prices = load_data(symbol, start_date, end_date)
...
macd_line, signal_line, macd_hist = calculate_macd(stock_prices)
...
sma, upper_band, lower_band = calculate_bollinger_bands(stock_prices)

# === 4. Generate Buy/Sell Signals ===
buy_signals = (stock_prices < lower_band) & (macd_line > signal_line)
sell_signals = (stock_prices > upper_band) & (macd_line < signal_line)

# === 5. Plotting ===
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10), sharex=True)

# Price with Bollinger Bands
ax1.plot(stock_prices, label=f'{symbol} Price', color='blue')
...
ax1.plot(stock_prices[buy_signals], '^', markersize=10, color='green', label='Buy Signal')
ax1.plot(stock_prices[sell_signals], 'v', markersize=10, color='red', label='Sell Signal')
ax1.set_title(f'{symbol} Price with Bollinger Bands & Buy/Sell Signals')
